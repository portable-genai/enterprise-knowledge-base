# Compliance : principle-to-control mapping (`enterprise-knowledge-base`)
...
the concrete controls implemented in **this** repo. `enterprise-knowledge-base` is the concrete home of **P-03**
...
ACL least-privilege (**P-09**). Where a principle does not apply to `enterprise-knowledge-base`, it is marked n/a
with a reason.

## A. General Principles (P-01..P-12)

| ID | Principle | Where it lives in `enterprise-knowledge-base` | Evidence |
| --- | --- | --- | --- |
...
| P-05 | Grounding over fine-tuning | `enterprise-knowledge-base` IS the governed RAG; answers are synthesised only from retrieved, permitted passages, never beyond the set; self-critique pass | `domain/kb_service.py`, `domain/prompts.py` |
...
| ID | Rule | `enterprise-knowledge-base` status | Control |
| --- | --- | --- | --- |
| R1 | Use `agent-guardrail-gateway` for screening + redaction | **Applies.** `enterprise-knowledge-base` ingests documents that may carry PII; `agent-guardrail-gateway` redaction/screening runs at the ingest and serve boundary | `GuardrailPort` / `PIIRedactionPort`; `adapters/platform/remote_guardrail.py`, `remote_redaction.py` |
| R2 | Audit to `agent-observability` | **Applies.** Every interaction writes an `AuditEvent` to `agent-observability` (or local WORM) | `AuditSinkPort`; `adapters/platform/remote_audit.py` |
| R3 | Use the `enterprise-knowledge-base` governed store | **It IS `enterprise-knowledge-base`.** `enterprise-knowledge-base` is the governed store other agents query; it does not consume itself | this repo |
| R4 | Register in `agent-registry` | **Open integration gate.** The deployed HTTP peer contract is discoverable, but AgentCard registration is deliberately disabled until Agent Runtime has a trusted invocation-context bridge | `api/contract.py`, `managed_readiness.tf`, `agent/root_agent.py` |
| R5 | Use the `model-quality-gate` before promotion | **Applies.** `enterprise-knowledge-base` ships a deterministic in-repo gate; `model-quality-gate` consumes the same report contract | `eval/run_eval.py`, `adapters/local/evaluation.py` |
...
6. Screen OUTPUT again (R1) and write an already-redacted WORM `AuditEvent` to `agent-observability` (R2, P-07)
...
| MAS FEAT / TRM 13: human accountability for a consequential output | Maker-checker floor: every synthesised answer carries `requires_human_review`, hard signals raise the level to enhanced, nothing auto-executes | `src/enterprise_kb/domain/hitl.py`, `tests/unit/test_policies.py` | Your reviewer roster and SLA in `human-review-console` |
