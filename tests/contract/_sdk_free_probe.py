...
Blocking is not the same as not installing. The dev environment installs no ``google-*``
distribution at all (they live in the ``[gcp]`` extra), so an absent SDK would make a hoisted
``from google.cloud import logging`` fail here for a reason that has nothing to do with the
prohibition, and the same probe would go green on a developer machine that happens to have the
extra installed. The finder below refuses the roots whatever is installed, and says so in the
message, so a failure can be told apart from a missing package.
...
interpreter in which the SDK was never importable.
...
#: Import roots the profiles must build without. ``google`` covers every ``google-cloud-*``
#: distribution plus ``google-genai``, ``google-adk`` and ``google-auth``; ``vertexai`` ships
#: as its own root.
