# logging_worm.tf : WORM audit trail: Cloud Logging WORM bucket + sink + audit config.
#
# General Principle map:
#   P-08 (immutable audit / WORM): the audit log is routed to a Cloud Logging bucket
#         whose retention is var.retention_days (~7 years) and which is Write-Once-Read-Many
#         when var.worm_locked = true. The variable has no default: see variables.tf. The audit adapter (cloud_logging_audit)
...
# # The variable has NO DEFAULT: every plan names it. A reference stack sets  # #
# # worm_locked = false and says why (recoverable, NOT a production posture). # #
# ############################################################################ #
...
resource "google_logging_project_bucket_config" "worm_audit" {
  ...
  description    = "Governed audit bucket for A2 enterprise knowledge base (WORM when worm_locked = true; ~7y floor binds when locked)."
  retention_days = var.retention_days # 2557 (~7 years) by default; the floor is conditional on the lock

  # IRREVERSIBLE when true: see WARNING banner above. WORM governance requires true, and the
  # variable has no default, so no plan can lock this bucket without the deployment saying so.
  # Production mode refuses below unless the deployment states the lock.
  locked = var.worm_locked

  lifecycle {
    precondition {
      condition     = !var.production_mode || var.worm_locked
      error_message = "production_mode requires worm_locked=true; the seven-year WORM lock is an explicit irreversible approval."
    }
  }
...
resource "google_logging_project_sink" "audit_to_worm" {
  ...
  description = "Routes the enterprise-knowledge-base-audit log to the WORM bucket."

  # Destination is the WORM logging bucket created above.
...
# destination remains the filtered WORM bucket above; no application workload gets this role.
