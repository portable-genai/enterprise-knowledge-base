...
//
// This console calls its service directly at `BASE_URL` from `lib/api` (the service's own
// origin standalone, a path on this origin behind a proxy), so the service lists both headers in
// its CORS `expose_headers` (`api/app.py`): a cross-origin response hides
// every header not listed there, and the pills would stay on the configured model forever with
// every test green. An empty base means same-origin root paths, and every one of them counts.
//
